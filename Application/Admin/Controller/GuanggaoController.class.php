...
class GuanggaoController extends Controller {
	public function index(){
		...
		$fri = M("column")->where(array("id"=>6,"is_del"=>1))->find();
		$this->assign("fri",$fri);
		$sec = M("column")->where(array("pid"=>6,"is_del"=>1))->select();
	...
	private function erji($erjiid){
		$botlink = M('article');
		$links = $botlink->where(array('is_del'=>1,'pid'=>$erjiid))->select();
		return $links;
	}
	public function editGuang(){
		$id = I("get.id");
		$col_id = M("column")->where(array("id"=>$id))->find();
		$this->assign("cld",$col_id);
		$this->display();
	}
	public function edit_guang(){
		$id = I("post.id");
		$data["title"] = I("post.title");
		$data["edit_time"] = time();
		$sql = M("column")->where(array("id"=>$id))->save($data);
		if ($sql) {
			$this->success("修改成功",U("Admin/Guanggao/index"),1);
		} else {
			$this->error("修改失败");
		}
	}
	...
	public function addNews(){
		$parent = M("column")->where(array("pid"=>6))->select();
	...
	public function add_news(){
		...
		$upload->subName   =	 "guanggaofuwu"; // 设置附件上传（子）目录
		$upload->autoSub   =	 true;	
		// 上传单个文件 
		...
		if ($sql) {
			$this->success("添加成功",U("Admin/Guanggao/index"),1);
		} else {
			$this->error("添加失败");
		}
	}
